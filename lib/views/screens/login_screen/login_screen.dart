...
class _LoginScreen extends State<LoginScreen> {
  ...
  Future firebaseLogin() async {
    ...
    try {
      ...
      // String id = result?.data?.login?.id ?? "";
      // String firstName = result?.data?.login?.firstName ?? "";
      // String lastName = result?.data?.login?.lastName ?? "";
      // String status = result?.data?.login?.status ?? "";
      // bool isVerified = result?.data?.login?.isVerified ?? false;
      // bool isActive = result?.data?.login?.isActive ?? false;
      // String gender = result?.data?.login?.gender ?? "";
      // String email = result?.data?.login?.email ?? "";
      // String phone = result?.data?.login?.phone ?? "";
      // String photoUrl = result?.data?.login?.photoUrl ?? "";
      ...
      // sharedPreferences.setBool("isVerified", isVerified);
      // sharedPreferences.setBool("isActive", isActive);
      // sharedPreferences.setString("gender", gender);
      // sharedPreferences.setString("email", email);
      // sharedPreferences.setString("phone", phone);
      // sharedPreferences.setString("photoUrl", photoUrl);

      // String firstName1 = sharedPreferences.getString("firstName") ?? "";
      //
      // print('firstName: $firstName1');
      // print(result?.data?.login?.id);
